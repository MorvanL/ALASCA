...
/**
 * The class <code>TestJavassistAdmissionControllerMonoJVM</code> deploys a test application for
 ...
 * 
 * The specificity of this test is that we use Javassist to create dynamically
 * the link between the interface required by the external application and 
 * the interface offered by the datacenter.
 * 
...
extends		AbstractCVM
{
	...
	public void			testScenario() throws Exception
	{
		for (int i = 0 ; i < NUMBER_OF_APPLICATIONS ; i++) {
			...
			AdmissionConnectionInformations connectionInformations = 
			...
			{	
				this.rgs.get(i).doPortConnection(
						...
						connectionInformations.getRequestSubmissionInboundPortUri(),
						connectionInformations.getConnectorClass().getCanonicalName()) ;
